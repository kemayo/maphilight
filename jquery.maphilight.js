(function($) {
  ...
  options_from_area = function(area, options) {
    var $area = $(area);
    // Get the area specific id
    var area_id = $(area).attr('id');
    if (options.areas) {
      if (options.areas[area_id]) {
        jQuery.each(defaultOptions, function(indexDefault, valueDefault) {
          // Go back to default options
          options[indexDefault] = defaultOptions[indexDefault];
          // Override specific options foreach area
          // For each area specific option
          jQuery.each(options.areas[area_id], function(index, value) {
            // If area value option is different from default value option
            if (indexDefault == index && valueDefault != value) {
              options[index] = value;
            }
          });
